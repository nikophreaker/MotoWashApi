...
<template>
  <div class="">
    ...
    <div class="mb-2">
      <TwInput class="bg-white rounded w-full p-2 focus:outline focus:outline-offset-2 focus:outline-sky-800" type="text"
    ...
    <TransitionGroup name="list" tag="ul" class="grid gap-2 grid-cols-[repeat(auto-fill,_minmax(14rem,_1fr))]">
      <li v-for="icon in filteredIcon" :key="icon" @click="copyText(icon)">
        <TwButton :icon="icon" variant="light" class="w-full text-sky-900 dark:text-gray-300" text-position="left" ripple>
          {{ icon }}
        </TwButton>
      </li>
    </TransitionGroup>
