...
export default defineEventHandler(async (event) => {
  try {
    ...
    const result = User.findOneAndUpdate({_id: body.id}, {name: body.name, email: body.email, phone: body.phone, licensePlates: body.licensePlates.split(","), loyaltyPoints: Number(body.loyaltyPoints)});
    // Admin.createIndexes();
